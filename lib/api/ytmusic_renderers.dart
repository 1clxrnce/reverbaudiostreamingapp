...
List<Map<String, dynamic>> _shelves(Map<String, dynamic> body) {
  ...
  if (tabs.isNotEmpty) {
    ...
    for (final c in contents) {
      final shelf =
          _asMap(c)['musicShelfRenderer'] ??
          _asMap(c)['musicCardShelfRenderer'];
...
Song? _parseItem(Map<String, dynamic> m) {
  ...
  // Artwork — last (largest) thumbnail, upscaled to 576×576.
  // YouTube thumbnail URLs end with a size token like =w226-h226 or =w60-h60.
  // Replacing it with =w576-h576 fetches the high-res version from the same CDN.
  String? artwork;
  final thumbs = _list(
...
/// Replaces YouTube's thumbnail size token with a 576×576 version.
/// Handles the =wNNN-hNNN suffix that YouTube Music uses on all thumbnails.
String _hqThumb(String url) {
  final sizeToken = RegExp(r'=w\d+-h\d+(-[^?]*)?$');
  if (sizeToken.hasMatch(url)) {
    return url.replaceFirst(sizeToken, '=w576-h576');
  }
  return url;
}
